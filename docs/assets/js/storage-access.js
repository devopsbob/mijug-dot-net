...
(function () {
  ...
  function isStorageAccessAPISupported() {
    if (isAPISupported === null) {
      isAPISupported = "requestStorageAccess" in document && "hasStorageAccess" in document;
    }
  ...
  function handleThirdPartyResources() {
    ...
    const calendarElements = document.querySelectorAll('[src*="calendar.google.com"], [href*="calendar.google.com"]');
    ...
    const iframes = document.querySelectorAll('iframe[src*="calendar.google.com"]');
    iframes.forEach((iframe) => {
      iframe.addEventListener("load", async () => {
        console.log("Google Calendar iframe loaded, requesting storage access...");
        await requestStorageAccess();
      });
    });
  }
  ...
  function setStoragePermissions() {
    ...
    if (!document.querySelector('meta[http-equiv="Permissions-Policy"]')) {
      ...
      meta.content = 'storage-access=(self "https://calendar.google.com" "https://www.google.com")';
        ...
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            const calendarElements = node.querySelectorAll
              ? node.querySelectorAll('[src*="calendar.google.com"], [href*="calendar.google.com"]')
              : [];

            calendarElements.forEach((element) => {
              element.addEventListener("click", async () => {
                console.log("Requesting storage access for dynamic Google Calendar content...");
                await requestStorageAccess();
              });
            });
          }
